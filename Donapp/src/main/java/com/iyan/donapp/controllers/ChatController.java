...
public class ChatController {
	...
	public String enviarMensaje(@RequestParam Long conversacionId, @RequestParam String contenido) {
	    Authentication auth = SecurityContextHolder.getContext().getAuthentication();
	    String email = auth.getName();
	    User remitente = userService.getUserByUsername(email);
	    Conversacion conversacion = conversacionesService.getConversacionById(conversacionId);
	    Mensaje mensaje = new Mensaje(conversacion, remitente, contenido);
	    mensajesService.save(mensaje);
	    conversacion.getMensajes().add(mensaje);
	    return "redirect:/chat/" + conversacionId;
	}
